...
public class UserDefineLibrary {
	...
	private static void initAmbiguityLibrary() {
		...
		if (file.isFile() && file.canRead()) {
			...
			LIBRARYLOG.info("init ambiguityLibrary ok path is : " + file.getAbsolutePath());
		} else {
...
//			loadLibrary(FOREST,baikeLibrary);
			...
			while ((temp = br.readLine()) != null) {
				if (StringUtil.isBlank(temp)) {
					...
					try{
						...
						System.out.println("load user-defined library failed , line at  " + i);
					}
				}
			}
	...
	public static void loadLibrary(Forest forest, String path) {
		...
		File file = null;
		if (path != null) {
			// load user defined library under project directory
			file = new File(path);
